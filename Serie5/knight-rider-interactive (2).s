...
main:
	BL	wiringPiSetupGpio		@ This will setup the wiringPi library.
	SUBS	R0, R0, #-1			@ In case something goes wrong, we exit the program
	BEQ	exit
...
waitForStartLoop:				@ Wait until the start button is pressed
	LDR	R0, .BUTTON2_PIN
	BL	digitalRead
	CMP	R0, #1
...
knightRider:
	...
	CMP	R0, #1
	BLNE	buzzerSound	// sound if not pressed
	CMP	R0, #1
	ADDEQ	R8, R8, #1	// increase score if pressed
	
	B 	continueLoop	

	ballMiddle:
	@ Check if button is pressed during animation
	LDR	R0, .BUTTON1_PIN
	MOV	R1, R7
	MOV	R2, R9
	BL	waitForButton
	MOV	R9, R1		// update button state (returned from subroutine)
	CMP	R0, #1
	BLEQ	buzzerSound	// sound if pressed
...
gameOver:
	@ Display score on LED bar (in binary)
	@ Latch pin low (read serial data)
	LDR	R0, .LATCH_PIN
	LDR	R1, .LOW
	BL	digitalWrite
	...
	B 	waitForStartLoop		@ When the game is over, we wait for a button press to restart the game.
