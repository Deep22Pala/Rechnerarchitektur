...
-----------------------------------------------------------
 ...
 Mikael Ruben Lechler 23-108-434
...
.global main
.func main
.extern printf //prints the score on the console
...
configurePins:
	...
	// Set the pins of BUTTON 1 to 'input' mode 
	/* to be implemented by student */
	LDR R0, .BUTTON1_PIN
	LDR R1, .INPUT
	BL pinMode
...
startCondition:
	// Check if button 2 is pressed to start the game
...
start:
	...
	*/
	Mov R4, #0b00000001		// R4: state of the LED bar(0-7)
...
knightRiderLoop:
	...
	*/
	...
	shiftLeft:
		MOV R4, R4 LSL #1 // Shift the LED bar state to the left
		B sendSerialData // Branch to sendSerialData
	
	shiftRight:
		MOV R4, R4 LSR #1 // Shift the LED bar state to the right
		B sendSerialData // Branch to sendSerialData
	...
	sendSerialData:
		LDR R0, .DATA_PIN
		LDR R1, .CLOCK_PIN
		LDR R2, .LSBFIRST // We are sending the most significant bit first
	...
	// Detect if button1 is pressed 
		...
		CMP R0, #1 // Check if ouptut in R0 is 1
		BEQ buttonCheck
		CMP R4,  #0b00000001 // Check if the LED 0 is on
		BEQ  alert
		CMP R4, #0b10000000 // Check if the LED 7 is on
		BEQ alert
	...
	buttonCheck:
		...
		BEQ addScore // If button was pressed in the given time window, branch to addScore
		BNE continue // If button was not pressed in the given time window, branch to alert
	...
	continue:
		...
		LDR R0, =scoreMessage // Load the scoreMessage to R0
		MOV R1, R10
		BL printf
		B knightRiderRestart // Start the knightRiderLoop again

gameOver:
		// Display the score on the LED bar
		LDR R0, =finalScore // Load the finalScore to R0
		BL printf // Calls the printf subroutine to display the final score
...
.data
scoreMessage:		.asciz	"Score: %d\n"
finalScore:		 	.asciz	"Final Score: %d\n"
